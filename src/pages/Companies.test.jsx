...
describe('Companies Component', () => {
  beforeEach(() => {
    useLoaderData.mockReturnValue({ companies: mockCompanies })
  })
  ...
  it('renders an empty table body when there are no companies', () => {
    useLoaderData.mockReturnValue({ companies: [] })
    render(<Component />)
...
describe('Companies loader', () => {
  it('returns cached companies from the store without calling the API', async () => {
    useStore.getState.mockReturnValue({ companies: mockCompanies, setCompanies: vi.fn() })
  ...
  it('fetches from the API and populates the store when the cache is empty', async () => {
    ...
    useStore.getState.mockReturnValue({ companies: [], setCompanies: setCompaniesMock })
    apiClient.get.mockResolvedValue({ data: mockCompanies })
